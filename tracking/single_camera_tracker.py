...
# )
# # Optical flow propagation removed – no import
...
#     Matches detections to existing tracks, creates new tracks, and deletes tracks
#     after too many consecutive missed detections. No optical flow propagation.
#     """
...
#         # Weighted sum: appearance (45%), spatial (50%), IoU (20%), minus age bias
#         return 0.45 * (1.0 - similarity) + 0.50 * centroid_distance + 0.2 * (1.0 - iou) - age_bias
...
#             ),
#             display_bbox=(
#                 tuple(float(v) for v in track.display_bbox)
...
#         # ----- Handle tracks that missed a detection: increment misses and delete if stale -----
#         stale_ids = []
#         for track_id, track in list(self.tracks.items()):
#             if track_id in assigned_tracks:
#                 continue

#             # Increment missed frame counter (no propagation attempt)
#             track.missed_frames += 1

#             # Delete track if it has missed too many consecutive frames
...
class SingleCameraTracker:
    ...
    def _make_observation(
        ...
        *,
        ...
        """Create a TrackObservation from a LocalTrack, evaluating ROI membership."""
        safe_roi_distance = signed_distance_to_polygon(track.centroid, safe_roi_polygon)

        # Determine ROI membership (override if provided, else point‑in‑polygon test)
        in_safe_roi = (
            track.in_safe_roi_override
            if track.in_safe_roi_override is not None
            else safe_roi_distance >= 0.0
        )
        in_outer_roi = (
            track.in_outer_roi_override
            if track.in_outer_roi_override is not None
        ...
        in_stable_roi = bool(
            ...
            or (
                in_outer_roi
                and safe_roi_distance >= -float(ROI_STABLE_EDGE_MARGIN_PX)
            )
        )
        ...
        return TrackObservation(
            ...
            safe_roi_distance=float(safe_roi_distance),
            temporal_iou=track.temporal_iou,
        )
    ...
    def update(
        ...
        *,
        ...
        # Generate observations only for tracks updated by a detection in this
        # frame. Missed local tracks stay alive for association, but emitting
        # their stale bbox/timestamp makes the global/event layers see ghost
        # detections.
        observations = [
            self._make_observation(
                track,
